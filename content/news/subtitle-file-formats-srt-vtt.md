...
A marketing manager receives the finished subtitles for a product video: twelve languages, all delivered as `.srt` files. The web team's player rejects them — it needs WebVTT. Someone renames the extension, and now half the videos play with no captions at all. Meanwhile the Arabic file opens in Notepad as backwards gibberish, and the Japanese one shows question marks where characters should be. None of this is exotic: subtitle files are plain text with timestamps, but that simplicity hides exactly the details — format capabilities, encoding, text direction — that decide whether your [multilingual subtitles](/news/subtitle-formatting-rules/) actually play. This is the working guide to the three formats you'll meet and the traps between them.
...
- **Keep CJK line counts short**: Chinese/Japanese/Korean convey more per character, so translated cues often shrink — an opportunity to raise reading comfort, per the [CJK subtitling notes](/news/subtitle-formatting-rules/).
